...
var Router = this.Router = Class.extend({
    ...
    match: function(url){
    	...
    	if(tmp){
    		...
    		tmp = util.extend({}, this.defaults);
    		result.data = util.extend(tmp, result.data);
    	}
    	return result;
    }
});
...
exports.getRouteData = function(url, method){
    var route = null,
        method = method ? method.toLowerCase() : 'get',
        // url: /blog/index?page=1 ,pathname is: /blog/index
        pathname = parseURL(url).pathname,
        r = new RouteResult(method),
        l = __routes.length;
        
    for(var i=0; i<l; i++){
        route = __routes[i].match(pathname);
        if(route.matched){
            r.controller = route.data.controller;
            r.action = route.data.action;
            r.args = util.extend({}, route.data);
