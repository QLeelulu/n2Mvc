...
//var MiddlewareFactory = function(req, res){
//    this.ctx = {req:req, res:res, ar:actionResults, handler:null};
//    this.eventIndex = -1;
//    events.EventEmitter.call(this);
//};
//MiddlewareFactory.prototype.next = function(){
//    this.eventIndex++;
//    var eventName = MIDDLEWARE_EXECUTE_ORDER[this.eventIndex];
//    if(!eventName){ return; }
//};
//
...
//    var _t = this, eventName = MIDDLEWARE_EXECUTE_ORDER[this.eventIndex];
//    if(eventName === 'beginRequest'){
//        this.ctx.routeData = route.getRouteData(this.ctx.req.urlPath, this.ctx.req.method);
//        if(this.ctx.routeData.isStaticFile){
//            this.ctx.handler = new StaticFileHandler(this.ctx.req, this.ctx.res, this.ctx.routeData.path);
//        }else{
//            this.ctx.handler = new MvcHandler(this.ctx.req, this.ctx.res, this.ctx.routeData);
//        }
//        this.ctx.handler.on('end', function(){
//            _t.next();
//        });
//    }
//    
//    this.eventIndex++;
//    eventName = MIDDLEWARE_EXECUTE_ORDER[this.eventIndex];
//    if(!eventName){ return; }
//    
//    if(eventName === 'processRequest'){
//        _t.ctx.handler.process();
//    }else{
//        if(eventName === 'beginMvcHandler' && ctx.routeData.isStaticFile){
//            _t.next();
//            return;
//        }
//        this.emit(eventName, _t.ctx, function(actionResult){
//            if(actionResult){
...
var MiddlewareFactory = {
    middleware_emitter: new events.EventEmitter(),
    /*
    ...
    nextMiddlewareEvent: function(i, ctx, callbackFn){
        MiddlewareFactory.middleware_emitter.emit('nextMiddlewareEvent', i, ctx, callbackFn);
    },
    nextMiddleware: function(eventName, i, ctx, callbackFn){
        MiddlewareFactory.middleware_emitter.emit('nextMiddleware', eventName, i, ctx, callbackFn);
    },
        ...
        if(eventName){
            if(eventName === 'processRequest'){
                ...
                if(eventName === 'beginMvcHandler' && ctx.routeData.isStaticFile){
                    ...
                    MiddlewareFactory.nextMiddlewareEvent(i, ctx, callbackFn);
                    return;
                }
                MiddlewareFactory.executeMiddlewares(eventName, ctx, function(actionResult){
                    if(actionResult){
                        ...
                        if(eventName === 'beginRequest'){
                            ...
                            ctx.handler.handlerEmitter.on('end', function(){
                                //i++;
                                //MiddlewareFactory.executeWrap(i, ctx, callbackFn);
                                MiddlewareFactory.nextMiddlewareEvent(i, ctx, callbackFn);
                            });
        ...
        if(__middlewares[i] && __middlewares[i][eventName]){
            __middlewares[i][eventName](ctx, function(result){
                if(result){
                    ...
                    MiddlewareFactory.nextMiddleware(eventName, i, ctx, callbackFn);
                    console.log('end nextMiddleware')
                }
            });
        }else{
            callbackFn();
...
MiddlewareFactory.middleware_emitter.on('nextMiddlewareEvent', function(i, ctx, callbackFn){
    i++;
    MiddlewareFactory.executeWrap(i, ctx, callbackFn);
});
MiddlewareFactory.middleware_emitter.on('nextMiddleware', function(eventName, i, ctx, callbackFn){
    i++;
    MiddlewareFactory.executeMiddleware(eventName, i, ctx, callbackFn);
});
...
var RequestHandler = exports.RequestHandler = Class.extend({
    init: function(req, res){
        this.req = req;
        this.res = res;
        this.handlerEmitter = new events.EventEmitter();
    },
    process: function(){
        throw new Error('Not Implement');
    },
    end: function(){
        this.handlerEmitter.emit('end');
    }
});
...
var ControllerFactory = exports.ControllerFactory = Class.extend({
    init: function(ctx){
        ...
        this.emitter.on('nextFilterEvent', function(i){
            i++;
            _t.executeWrap(i);
        });
        this.emitter.on('nextFilter', function(filters, filterEventName, i, fn){
            i++;
            _t.executeWrap(filters, filterEventName, i, fn);
        });
        this.ctx = ctx;
    ...
    nextFilterEvent: function(i){
        this.emitter.emit('nextFilterEvent', i);
    },
    nextFilter: function(filters, filterEventName, i, fn){
        this.emitter.emit('nextFilter', filters, filterEventName, i, fn);
    },
        ...
        if(filters[i] && filters[i][filterEventName]){
            ...
            filters[i][filterEventName](_t.ctx, function(result){
                if(result){
                    ...
                    _t.nextFilter(filters, filterEventName, i, fn);
                }
            });
        }else{
            fn();
