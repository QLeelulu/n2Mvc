        ...
        middlewares.forEach(function(m){
            ...
            if(typeof m === 'string'){
                try{
                    ...
                    try{
                        console.log( Settings.projectDir );
...
var ControllerFactory = exports.ControllerFactory = Class.extend({
    init: function(ctx){
        ...
        if(routeData.controller && routeData.action){
            ...
            ctx.req.error = new Error('404: No route was matched');
            ctx.req.canceled = true;
        }
    },
    checkAction: function(){
        ...
        if(!this.controller){
            this.ctx.req.error = new Error('404: No controller called "' + this.ctx.routeData.controller + '"');
            this.ctx.req.canceled = true;
            return false;
        }
        if(!this.action){
            this.ctx.req = new Error('404: The "' + this.ctx.routeData.controller + '" controller doesn\'t have an action called "' + this.ctx.routeData.action + '"');
            this.ctx.req.canceled = true;
            return false;
        }
